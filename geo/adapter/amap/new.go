...
func New(opts ...Option) (*Locator, error) {
	...
	// 与 http/baidu 适配器一致：<=0 回落默认 5s，避免无限挂起
...
// MustNew 创建新的高德 IP 定位适配器，失败时使用 log.Fatalf 退出。
func MustNew(opts ...Option) *Locator {
	locator, err := New(opts...)
	if err != nil {
		log.Fatalf("amap geo: create locator failed: %v", err)
	}
	return locator
}
...
func validateOptions(o Options) error {
	if strings.TrimSpace(o.Key) == "" {
		return geo.NewError(geo.ErrCodeInvalidArgument, "amap key is required")
	}
