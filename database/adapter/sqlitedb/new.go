...
// MustNew 创建由 SQLite 支持的 database.DB，失败时 panic。
func MustNew(opts ...Option) database.DB {
	db, err := New(opts...)
	if err != nil {
		panic(fmt.Errorf("sqlitedb: create database failed: %w", err))
	}
	return db
}
...
func MustNewWithConfig(cfg config.Config, prefixes ...string) database.DB {
	...
	if err != nil {
		panic(fmt.Errorf("sqlitedb: create database from config failed: %w", err))
	}
	return db
}
