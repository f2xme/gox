...
func New(opts ...Option) (database.DB, error) {
	...
	return wrapPostgres(base)
}

// MustNew 创建由 PostgreSQL 支持的 database.DB，失败时 panic。
func MustNew(opts ...Option) database.DB {
	db, err := New(opts...)
	if err != nil {
		panic(fmt.Errorf("pgsqldb: create database failed: %w", err))
	}
	return db
}
...
func MustNewWithConfig(cfg config.Config, prefixes ...string) database.DB {
	...
	if err != nil {
		panic(fmt.Errorf("pgsqldb: create database from config failed: %w", err))
	}
	return db
}

func wrapPostgres(base database.DB) (database.DB, error) {
	g, ok := base.(*gormbase.GormDB)
	if !ok {
		return nil, fmt.Errorf("pgsqldb: unexpected database type %T", base)
	}
	log.Println("pgsqldb: database connected successfully")
	return &PostgresDB{GormDB: g}, nil
}
